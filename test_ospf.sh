...
test_ospf_neighbors() {
    ...
    # Get OSPF neighbor count from SR Linux
    neighbor_output=$(docker exec clab-ospf-mpls-backbone-$router sr_cli "show network-instance default protocols ospf neighbor" 2>/dev/null)
...
test_ospf_routes() {
    ...
    route_output=$(docker exec clab-ospf-mpls-backbone-$router sr_cli "show network-instance default route-table" 2>/dev/null)

    if [ $? -eq 0 ]; then
        route_count=$(echo "$route_output" | grep -c "ospf" 2>/dev/null || echo "0")
